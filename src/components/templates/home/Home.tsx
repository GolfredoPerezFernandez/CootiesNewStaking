...
const Home = (props: any) => {
  const { Moralis, user, isWeb3Enabled, isAuthenticated,web3, isWeb3EnableLoading } = useMoralis();
...
tokenIds=[...tokenIds,res3[i][1]];
  ...
  const handleWithdrawStakingV2New = async () => {
    ...
    try{
		...
		const Proposals = Moralis.Object.extend('ItemsStaked');
		const query = new Moralis.Query(Proposals);
		query.equalTo('owner',user?.get('ethAddress'));
		
		query.descending('createdAt');
		const results = await query.first();
		console.log(results?.attributes.createdAt)
		console.log(results?.attributes.tokensId)
    const sendOptions1 = {
...
tokenIds=[...tokenIds,res3[i][1]];
  ...
  useEffect(() => { 
	setWeb3jsProvider(Moralis.provider)

    async function init() {       
      const chainId2 = await Moralis.getChainId();
       if (chainId2 === '0x13') {
        const options3 = {
        ...
        const options5 = {
		  ...
		  const totalSupplyNFt5: any = await Moralis.executeFunction(options55);
	  ...
	  setRewardsNew(totalSupplyNFt5.toString())
	 await handleUpdateStakingV2New()
	 await handleUpdateStakingV1()
	 await handleUpdate()
	 await handleUpdate2()
	 await handleUpdate3()
	 await handleUpdate4()
	 ...
	 await axios.get(`https://songbird-explorer.flare.network/api?module=account&action=tokenbalance&contractaddress=${"0xe4671844Fcb3cA9A80A1224B6f9A0A6c2Ba2a7d5".toLowerCase()}&address=${"0x48d93BfB7864305aD85178036abE3B07c4B6e6a5".toLowerCase()}`,{
        responseType: 'json'
      })
.then(async (res:any) => {
if(res.status === 200) { 
	...
	setNftV2TVL((parseFloat(Moralis.Units.FromWei(res.data.result))).toString())
	 ...
	 await axios.get(`https://songbird-explorer.flare.network/api?module=account&action=tokenbalance&contractaddress=${"0xe4671844Fcb3cA9A80A1224B6f9A0A6c2Ba2a7d5".toLowerCase()}&address=${"0x9bc49C925ae38F97c28BfbCe2d0aD33cd24AeA8E".toLowerCase()}`,{
        responseType: 'json'
      })
.then(async (res:any) => {
if(res.status === 200) { 
...
if(res.status === 200) { 
	setTVL3(Moralis.Units.FromWei(res.data.result))
	 ...
	 await axios.get(`https://songbird-explorer.flare.network/api?module=account&action=tokenlist&address=${"0x9bc49C925ae38F97c28BfbCe2d0aD33cd24AeA8E".toLowerCase()}`,{
        responseType: 'json'
      }).then(async (res:any) => {
if(res.status === 200) { 
...
await axios.get(`https://songbird-explorer.flare.network/api?module=account&action=tokenlist&address=${"0x48d93BfB7864305aD85178036abE3B07c4B6e6a5".toLowerCase()}`,{
        responseType: 'json'
      })
.then(async (res:any) => {
if(res.status === 200) { 
...
console.log("lista"+lista[0].balance)
  ...
  const handleDepositStakingV2New = async () => {
    try {
      if (user) {
        ...
        const res0 = await contract0
          .connect(signer)
          .setApprovalForAll('0x48d93BfB7864305aD85178036abE3B07c4B6e6a5',true);
        ...
        await contractProcessor.fetch({
		  ...
		  const Item = Moralis.Object.extend("ItemsStaked")
...
if(res.status === 200) { 
  ...
  const handleUpdate5 = async () => {
    try {
      ...
      const pen = await contract.connect(signer).pendingReward(0, user?.get('ethAddress'));
	  const rew = await contract.connect(signer).rewardPerSecond();
	  console.log(Moralis.Units.FromWei(rew))
...
<Heading   color={"#1CFFA0T"}  width={props.width<1000?"280px":"100%"} marginBottom={'20px'}  marginTop={props.width<1000?"80px":"0px"} fontSize={props.width<1000?"2xl":"4xl"}  alignSelf={'center'} textAlign={'center'}>
  ...
  >
      ...
      sequence={[
        ...
        4000,
        'Yield Optimizer on Songbird ',
        4000,
        "Genesis holders can claim 20% of CootieMart's trading fees",
		4000
      ]}
      wrapper="div"
      cursor={true}
